...
void
bloop_ready(bloops *B, bloopsalive *A, unsigned char init)
{
...
bloopsalive *
bloop_play(bloops *B, bloopsaphone *P)
{
...
static void
bloop_synth(bloops *B, int length, float* buffer)
{
  int i, si;
    ...
    bloopsalive *A = B->live;
      ...
      A->filter[6] *= A->filter[7];
		...
		for (si = 0; si < 8; si++)
		{
			...
			float fp = (float)A->phase / A->period;
			switch (A->P->type)
			{
        case BLOOPS_SQUARE:
          if (fp < A->square)
  ...
  PaStreamCallbackFlags statusFlags, void *data)
{
  ...
  bloops *B = (bloops *)data;
	...
	if (B->play == BLOOPS_PLAY)
		bloop_synth(B, framesPerBuffer, out);
	else
